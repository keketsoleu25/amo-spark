...
export default function Home() {
  return (
    <main>
      <header className="site-header">
        ...
        <details className="mobile-menu">
          ...
          <nav className="mobile-nav" aria-label="Mobile navigation">
            <a href="#shop">Shop</a><a href="#cleaning">Cleaning products</a><a href="#diffusers">Room diffusers</a><a href="#about">About</a><a href="#contact" className="mobile-order">Order / Contact</a>
          </nav>
        </details>
      </header>

      <section className="hero" id="top">
        <div className="hero-copy"><p className="eyebrow">Cleaning essentials · Home fragrance</p><h1>A fresh home starts with <em>Amo Spark.</em></h1><p className="hero-text">Everyday cleaning products and room diffusers made to keep your space clean, fresh and welcoming.</p><div className="hero-actions"><a href="#cleaning" className="button">Shop cleaning</a><a href="#diffusers" className="button button-ghost">Shop diffusers</a></div><div className="trust-row"><span>Locally owned</span><span>Everyday freshness</span><span>Easy ordering</span></div></div>
      ...
      <section className="section products" id="cleaning"><div className="section-heading split-heading"><div><p className="eyebrow">Cleaning products</p><h2>The everyday clean.</h2></div><p>Replace these placeholders with Amo Spark's actual products, sizes and prices.</p></div><div className="product-grid">{products.slice(0, 3).map((product, index) => <article className="product-card" key={`${product.name}-${index}`}><ProductImage label={product.name}/><div className="product-meta"><span>{product.category}</span><span>{product.price}</span></div><h3>{product.name}</h3><p>{product.size}</p><a href="#contact" className="text-link">Order this product →</a></article>)}</div></section>

      <section className="section products scent-section" id="diffusers"><div className="section-heading split-heading"><div><p className="eyebrow orange">Room diffusers</p><h2>Make home smell like home.</h2></div><p>Add the real diffuser names, scent notes, sizes and prices when the product catalogue is ready.</p></div><div className="product-grid">{products.slice(3).map((product, index) => <article className="product-card" key={`${product.name}-${index}`}><ProductImage label={product.name}/><div className="product-meta"><span>{product.category}</span><span>{product.price}</span></div><h3>{product.name}</h3><p>{product.size}</p><a href="#contact" className="text-link">Order this diffuser →</a></article>)}</div></section>
